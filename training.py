...
def fetch_from_kaggle(dest_path=None):
    """
    Fetches the latest dataset from Kaggle using the CLI. Falls back to local backup if Kaggle fetch fails.
    Returns a status message.
    """
    # Use config for Kaggle settings and paths
    dataset, filename = config.get_kaggle_config()
    if dest_path is None:
        dest_path = config.get_data_path()
    
    try:
        # Download using Kaggle CLI
        kaggle_cmd = [
            "kaggle", "datasets", "download", "-d", dataset, "-f", filename, 
            "-p", config.config["paths"]["data_dir"], "--force"
        ]
        result = subprocess.run(kaggle_cmd, capture_output=True, text=True, timeout=60)
        if result.returncode != 0:
            raise Exception(result.stderr)
        # Unzip if needed
        zip_path = os.path.join(config.config["paths"]["data_dir"], filename.replace(".csv", ".zip"))
        if os.path.exists(zip_path):
            import zipfile
            with zipfile.ZipFile(zip_path, 'r') as zip_ref:
                zip_ref.extractall(config.config["paths"]["data_dir"])
            os.remove(zip_path)
...
def restore_backup(backup_file, dest_path=None):
    """
